...
for i:=1 to d do
	...
	for j:=(i-1) down to 0 do
		if((window-id is even AND j th bit of Pk = 0) OR 
		   (window-id is odd AND j th bit of Pk = 1)) then 
		   call CompareLow(j)
		else
			call CompareHigh(j)
		endif
...
void bitonic(SortType array[], const FSize size, const int np, const int rank)
{
    ...
    const int logNp = log( (double)np ) / log( 2.0 );
...
void print(const long long array[], const int size, const int rank)
{
