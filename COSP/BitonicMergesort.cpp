...
namespace COSP
{
	...
	BitonicMergesort::BitonicMergesort(int& argc, char**& argv)
	{
		MPIBase::Init(argc,argv);
	}

	BitonicMergesort::~BitonicMergesort(void)
	{
		MPIBase::Finalize();
	}

	void BitonicMergesort::RunMaster(std::vector<int>& vnr){
	}
	void BitonicMergesort::RunSlave(std::vector<int>& vnr){
	}

	void BitonicMergesort::Run(std::vector<int>& vnr){
